...
export const GlowingLyricEffect = (p: p5, lyrics: Array<{ start: number; end: number; text: string }>, isPlaying: boolean, currentTime: number) => {
    if (!isPlaying) return;
    ...
    if (currentLyric) {
            ...
            words = lines.map(line => 
              line.split(' ').map(word => {
                const wordObj = {
                  ...
                  endTime: wordStartTime + wordDuration * 2 // Double the duration to allow overlap
                };
        ...
        words.forEach((line, lineIndex) => {
            ...
            line.forEach((word, wordIndex) => {
                const halfDuration = (word.endTime - word.startTime) / 2;
                const midPoint = word.startTime + halfDuration;

                if (currentTime >= word.startTime && currentTime <= word.endTime) {
                    if (currentTime <= midPoint) {
                        // Fade in
                        word.opacity = p.map(currentTime, word.startTime, midPoint, 0, 255);
                    } else {
                        // Fade out
                        word.opacity = p.map(currentTime, midPoint, word.endTime, 255, 0);
                    }
